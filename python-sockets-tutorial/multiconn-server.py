...
def service_connection(key, mask):
    ...
    !!!Обратите внимание, что при этом сервер зависит от
    хорошего поведения клиента: сервер ожидает, что клиент
    закроет свою сторону соединения, когда он закончит
    отправлять сообщения. Если клиент не закрывается, сервер
    оставит соединение открытым. В реальном приложении вы
    можете защитить свой сервер от этого и предотвратить
    накопление клиентских подключений, если они не отправят
    запрос по прошествии определенного времени.
    """
    sock = key.fileobj
    data = key.data

    # Если сокет готов к чтению, то mask & selectors.EVENT_READ истинны
    if mask & selectors.EVENT_READ:
        # вызывается sock.recv().
        recv_data = sock.recv(1024)  # Должен быть готов к чтению
        if recv_data:
            # Любые считанные данные добавляются к data.outb,
            # чтобы их можно было отправить позже.
            data.outb += recv_data
        else:
            # Обратите внимание на блок else:, если данные не получены
            print("Закрытие соединения с", data.addr)

            # Это означает, что клиент закрыл свой сокет,
            # поэтому сервер тоже должен это сделать.
            # Но не забудьте сначала вызвать sel.unregister (), чтобы он больше не контролировался select().
            sel.unregister(sock)
            sock.close()

    if mask & selectors.EVENT_WRITE:
        # Когда сокет готов к записи, что всегда должно иметь место для
        # здорового сокета,
        if data.outb:
            print("Эхо", repr(data.outb), "для", data.addr)

            # любые полученные данные, хранящиеся в data.outb,
            # эхом передаются клиенту с помощью sock.send().
            sent = sock.send(data.outb)  # Должен быть готов к записи

            # Затем отправленные байты удаляются из буфера отправки:
